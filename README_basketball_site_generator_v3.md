...
- `index.html`: home dashboard — latest results (with records), standings (playoff cutoff, SOS, last-10 dots, day-over-day movement arrows), playoff odds (5,000 Monte-Carlo season sims with full seed distributions), league news feed, injury report, league leaders, rookie watch, heat-mapped team stats, Four Factors, and award voting sentiment.
...
- `draft.html`: tabbed draft classes (2029/2030/2031) with heat-mapped prospect ratings (each prospect gets a page) plus the projected draft order with pick ownership per year.
...
- `records.html`: all-time career leaderboards (2026-present, including retired players) and every notable single-game feat.
...
- Global extras on every page: search box (press `/` to focus) and stat-glossary tooltips on column headers.
